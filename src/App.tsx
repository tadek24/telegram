  ...
  return <div className="modal-layer install-guide-layer" role="presentation">
    <section className="install-guide-modal" role="dialog" aria-modal="true" aria-labelledby="install-guide-title">
      ...
      <footer className="install-guide-footer">
        <p><Icon name="lock" /><span><strong>Po instalacji</strong> Otwórz ikonę aplikacji i zaloguj się numerem telefonu oraz własnym hasłem. {invited ? 'To zaproszenie zastępuje kod dostępu podczas tworzenia konta.' : 'Kod dostępu jest potrzebny tylko podczas tworzenia nowego konta.'}</span></p>
...
function friendlyError(error: unknown) {
  ...
  if (code.includes('INVALID_CREDENTIALS')) return 'Nieprawidłowy numer telefonu lub hasło.'
  if (code.includes('WEAK_PASSWORD')) return 'Hasło musi mieć co najmniej 4 znaki.'
  ...
  return <main className="login-page">
    <section className="login-card">
      ...
      {showPhoneLogin ? <form className="demo-login-form" onSubmit={submitPhoneLogin}>
        ...
        <label>Hasło <small>(minimum 4 znaki)</small><input required minLength={4} maxLength={128} type="password" autoComplete="current-password" value={password} onChange={event => setPassword(event.target.value)} /></label>
      ...
      <p className="privacy-note">{showDemoLogin ? 'To lokalny PIN demonstracyjny. Profil, kontakty i rozmowy są zapisywane wyłącznie na tym urządzeniu.' : 'Połączenie jest chronione, a hasło nie jest zapisywane w przeglądarce.'}</p>
...
export default function App() {
  ...
  const activeRoomCanCall = Boolean(activeRoom && activeRoom.getJoinedMembers().length === 2)
  ...
  const mediaItems = (activeRoom?.getLiveTimeline().getEvents() ?? [])
    ...
    .map(event => ({ event, content: event.getContent<MatrixMediaContent>(), name: event.getContent<{ body?: string, filename?: string }>().filename || event.getContent<{ body?: string }>().body || 'Załącznik' }))
    .sort((a, b) => b.event.getTs() - a.event.getTs())
  const visualMediaItems = mediaItems.filter(({ content }) => content.msgtype === 'm.image' || content.msgtype === 'm.video')
  const fileMediaItems = mediaItems.filter(({ content }) => content.msgtype === 'm.audio' || content.msgtype === 'm.file')
  ...
  async function openRoomMedia() {
    ...
    setMediaLoadError('')
    try {
      await client.scrollback(activeRoom, 1000)
    } catch {
      setMediaLoadError('Nie udało się pobrać starszych załączników. Pokazujemy media, które są już zsynchronizowane.')
    } finally {
  ...
  return <main className="workspace" data-chat-theme={theme}>
    ...
    {roomSettingsDialog && activeRoom && <div className="modal-layer" role="presentation"><section className="modal room-settings-modal" role="dialog" aria-modal="true" aria-labelledby="room-settings-title">
      ...
      <section className="room-media-entry"><button type="button" onClick={() => void openRoomMedia()}><span><Icon name="files"/></span><span><strong>Wyświetl pliki i media</strong><small>Tylko z rozmowy „{displayRoomName(activeRoom)}”</small></span><b aria-hidden="true">›</b></button></section>
      {activeRoomIsGroup ? <>
...
function DemoWorkspace({ phone, onLogout }: { phone: string, onLogout: () => void }) {
  ...
  const [conversationMediaDialog, setConversationMediaDialog] = useState(false)
  ...
  const demoMediaItems = active?.messages.filter(message => message.attachment) ?? []
